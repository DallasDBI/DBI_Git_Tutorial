...
As always, `git add` and `git commit` your file, then open up another
Git Bash window in your repo folder and enter in `git checkout main`.
Try typing in `git log` in both instances of Git Bash. Comparing the
output of the two should show that the **bugfix** window lists the
commit you just made, but the **main** window does not.

![](img/gitbranchcomparison.png)
...
Now if we use `git log` in our **main** branch window, the commit
histories will be identical.

![](img/gitbranchcomparisonmerged.png)

We have now learned the basics of creating and merging branches in Git
